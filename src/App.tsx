...
export default function App(){
  ...
  return <main>
    ...
    <section className="layout"><div className="gameWrap"><Game onGameOver={gameOver} restartSignal={restartSignal}/><div className="actions"><div><b>Last</b><strong>{last}</strong></div><div><b>Best</b><strong>{best}</strong></div>{gameEnded&&<div className="gameButtons"><button onClick={submit} disabled={!connected||saving||scoreSaved||last<=0}>{saving?'Saving…':scoreSaved?'Saved on-chain':'Save score on-chain'}</button><button className="secondary" onClick={playAgain} disabled={saving}>Play again</button></div>}</div>{tx&&<p className="tx">✓ Submitted: <a href={explorerTxUrl(tx)} target="_blank" rel="noreferrer">{tx.slice(0,12)}… <span className="external">↗</span></a></p>}</div><aside><h2>Leaderboard</h2>{rows.length===0?<p className="muted">Deploy Move module to activate on-chain scores.</p>:rows.map((r,i)=><div className="row" key={r.player}><span>#{i+1}</span><code>{r.player.slice(0,6)}…{r.player.slice(-4)}</code><b>{r.score}</b></div>)}<div className="network"><i></i>Shelbynet • Petra Wallet • On-chain</div></aside></section>

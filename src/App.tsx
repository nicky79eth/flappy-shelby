...
export default function App(){
  ...
  return <main>
    ...
    <section className="layout">
      <div className="gameWrap"><Game onGameOver={gameOver}/><div className="actions"><div><b>Last</b><strong>{last}</strong></div><div><b>Best</b><strong>{best}</strong></div><button onClick={submit} disabled={!connected||last<=0||saving}>{saving?'Saving…':'Save score on-chain'}</button></div>{tx&&<p className="tx">✓ Submitted: <a href={explorerTxUrl(tx)} target="_blank" rel="noreferrer" title="View transaction on Aptos Explorer">{tx.slice(0,12)}… <span className="external">↗</span></a></p>}</div>

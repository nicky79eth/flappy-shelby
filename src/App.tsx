...
function FlyingShelbyMark(){
  const wing=(side:number)=><g transform={`translate(42 22) scale(${side} 1)`}><path d="M14-3 Q25-17 34-22 Q31-12 27-7 Q34-11 39-9 Q33-2 25 3 Q31 1 35 4 Q27 9 16 9Z"/><path d="M18-1 Q27-10 33-15M19 3Q27 0 33-4"/></g>;
  return <svg className="titleLogo" viewBox="0 0 84 44" aria-hidden="true"><g fill="rgba(255,92,173,.16)" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">{wing(-1)}{wing(1)}</g><image href={SHELBY_LOGO} x="21" y="3" width="42" height="38" preserveAspectRatio="xMidYMid meet"/></svg>;
}
...
export default function App(){
  ...
  return <main>
    ...
    <section className="layout"><div className="gameWrap"><Game onGameOver={gameOver} restartSignal={restartSignal}/><div className="actions"><div><b>Last</b><strong>{last}</strong></div><div><b>Best</b><strong>{best}</strong></div>{gameEnded&&<div className="gameButtons"><button onClick={submit} disabled={!connected||saving||scoreSaved||last<=0}>{saving?'Saving…':scoreSaved?'Saved on-chain':'Save score on-chain'}</button><button className="secondary" onClick={playAgain} disabled={saving}>Play again</button></div>}</div>{tx&&<p className="tx">✓ Submitted: <a href={explorerTxUrl(tx)} target="_blank" rel="noreferrer">{tx.slice(0,12)}… <span className="external">↗</span></a></p>}</div><aside><h2>Leaderboard</h2>{rows.length===0?<p className="muted">Deploy Move module to activate on-chain scores.</p>:rows.map((r,i)=><div className="row" key={r.player}><span>#{i+1}</span><code>{r.player.slice(0,6)}…{r.player.slice(-4)}</code><b>{r.score}</b></div>)}<div className="network"><img src={SHELBY_LOGO} alt="" aria-hidden="true"/>Shelbynet • Petra Wallet • On-chain</div></aside></section>
